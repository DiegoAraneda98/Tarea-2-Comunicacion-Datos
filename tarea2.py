...
# Función para verificar si una secuencia de bits contiene errores utilizando CRC-CCITT
def verificar_crc_ccitt(datos, polinomio):
    crc = datos[-len(polinomio):]
    datos = datos[:-len(polinomio)]
    return calcular_crc_ccitt(datos, polinomio) == crc
...
for archivo in archivos_dataset2:
    ...
    es_correcto = verificar_crc_ccitt(datos, polinomio_crc)
    if es_correcto:
